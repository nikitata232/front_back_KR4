...
> **Разделы:** [Быстрый старт](#быстрый-старт) · [Тестирование](#тестирование) · [Postman](#postman--примеры-для-демонстрации-балансировщика) · [Redis](#как-работает-redis-кэширование) · [Nginx](#как-работает-балансировка-nginx) · [Демо-аккаунты](#демо-аккаунты-для-всех-практик) · [API](#api-общий-для-всех-практик) · [Технологии](#технологии)
...
for i in 1 2 3 4 5 6; do curl -s http://localhost:8022/health; echo; done
```
...
Открой в браузере — наглядно видно балансировку и кэш в реальном времени:

```
http://localhost:3021/dashboard.html
```

---

### Postman — примеры для демонстрации балансировщика

**1. Health check** — отправь несколько раз, поле `server` чередуется, в Headers виден `X-Server-Id`
...
Ответ:
```json
{ "status": "ok", "server": "1", "db": "connected", "redis": "connected" }
```

**2. Список товаров** — первый запрос `source: db`, второй `source: cache` (Redis общий для всех backends)

```
GET http://localhost:8022/api/products
```

**3. Логин** — получить JWT токен

```
POST http://localhost:8022/api/auth/login
...
**4. Создать товар** — данные попадут в общую БД, видны через любой backend

```
POST http://localhost:8022/api/products
Authorization: Bearer <токен из шага 3>
Content-Type: application/json
...
После создания сделай `GET /api/products` — товар виден независимо от того, какой backend ответил.

> В Postman после каждого запроса открывай вкладку **Headers** в ответе — там заголовок `X-Server-Id` показывает какой именно backend обработал запрос.
