...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://localhost:27017/phonestore';
...
const swaggerSpec = swaggerJsdoc({
  definition: {
    openapi: '3.0.0',
    info: { title: 'PhoneStore API (MongoDB)', version: '1.0.0', description: 'RBAC: роли user / seller / admin. Хранение в MongoDB.' },
...
app.get('/', (req, res) => {
  res.json({ message: 'PhoneStore API (MongoDB)', port: PORT, docs: '/api/docs' });
});
...
// Helper: safe user object (without hashed_password)
const safeUser = (u) => ({
  id: u.id,
  email: u.email,
...
async function seedData() {
  for (const u of DEMO_USERS) {
    ...
    if (!exists) {
      const hashed_password = await bcrypt.hash(u.password, SALT_ROUNDS);
      await User.create({
        id: nanoid(),
        email: u.email,
        first_name: u.first_name,
  ...
  if (count === 0) {
    for (const p of SEED_PRODUCTS) {
      await Product.create({ id: nanoid(), ...p });
    }
...
app.get('/api/products', async (req, res) => {
  try {
    ...
    res.json(products.map(p => ({ id: p.id, title: p.title, category: p.category, description: p.description, price: p.price, image: p.image })));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.get('/api/products/:id', async (req, res) => {
  try {
    ...
    res.json({ id: p.id, title: p.title, category: p.category, description: p.description, price: p.price, image: p.image });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/products', authMiddleware, roleMiddleware('seller', 'admin'), async (req, res) => {
  try {
    ...
    res.status(201).json({ id: p.id, title: p.title, category: p.category, description: p.description, price: p.price, image: p.image });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.put('/api/products/:id', authMiddleware, roleMiddleware('seller', 'admin'), async (req, res) => {
  try {
    ...
    const p = await Product.findOneAndUpdate(
      { id: req.params.id },
      { title, category, description, price: Number(price), image },
      { new: true }
    );
    if (!p) return res.status(404).json({ error: 'Товар не найден' });
    res.json({ id: p.id, title: p.title, category: p.category, description: p.description, price: p.price, image: p.image });
...
app.delete('/api/products/:id', authMiddleware, roleMiddleware('admin'), async (req, res) => {
  try {
    const p = await Product.findOneAndDelete({ id: req.params.id });
    if (!p) return res.status(404).json({ error: 'Товар не найден' });
...
mongoose
  ...
  .then(async () => {
    ...
    await seedData();
    app.listen(PORT, () => console.log(`PhoneStore (MongoDB) running on http://localhost:${PORT}`));
  })
  .catch((err) => {
