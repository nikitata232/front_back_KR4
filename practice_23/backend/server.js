...
app.get('/', (req, res) => {
  res.json({ message: 'PhoneStore API (Docker)', server: SERVER_ID, port: PORT });
});
...
async function start() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`PhoneStore Docker server-${SERVER_ID} (PostgreSQL + Redis) running on http://0.0.0.0:${PORT}`);
  });
}
